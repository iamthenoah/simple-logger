...
module.exports = class Logger {
	...
	processLog(logElement, ...info) {
		...
		info.forEach(t => {
			...
			const isData = typeof t === 'object' && !Array.isArray(t)
			if (isData) t = flatten(t)
			const n = isData ? Object.keys(t).map(k => k + '=>' + t[k]) : truncate(t.toString(), m * 0.075, m)
			n.forEach(l => this.write(label + style.colorLog(prefix ? prefix + ' ' + l : l)))
		})
