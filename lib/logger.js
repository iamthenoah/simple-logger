...
module.exports = class Logger {
	...
	label(label, style, ...info) {
		info.forEach(i => this.process(new Log(i, label, style)))
	}

	write(log) {
		const p = `[${new Date().toLocaleString()}] ${Styles.Debug.colorForeground('::')}`
		process.stdout.write(`${p} ${log ? log : ''}\n`)
	}
	...
	process(log) {
		...
		if (info.length <= process.stdout.columns - 40) {
			this.write(tag + ' ' + (style ? style.colorForeground(info) : info))
		} else {
			const chunks = info.match(new RegExp('.{1,' + (process.stdout.columns - 50) + '}', 'g'))
			chunks.forEach(l => this.write(tag + ' ' + (style ? style.colorForeground(l) : l)))
		}
	}
}
